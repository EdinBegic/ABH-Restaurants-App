...
export default BaseRoute.extend({
  ...
  actions: {
    ...
    formatPageNumbering(response, pageNumber) {
      let numOfPages = Math.ceil(response.available / CONSTANTS.POPULAR_RESTAURANTS_SIZE);
      this.controller.set('numOfPages', this.createPageArray(numOfPages,pageNumber));
    ...
    didTransition() {
      ...
      this.get("_restaurantService").filter(null, null, 1, CONSTANTS.POPULAR_RESTAURANTS_SIZE)
      .then(response => {
